
<aside class="admin-sidebar">
    <ul>
        <li><a href="<?php echo $base_url; ?>admin/index.php">📊 Dashboard</a></li>
        <li><a href="<?php echo $base_url; ?>admin/banners.php">🖼 Banners</a></li>
        <li><a href="<?php echo $base_url; ?>admin/products.php">📦 Products</a></li>
        <li><a href="<?php echo $base_url; ?>admin/manage-offers.php">🏷️ Offers</a></li>        
        <li><a href="<?php echo $base_url; ?>admin/orders.php">🛒 Orders</a></li>
        <li><a href="<?php echo $base_url; ?>admin/messages.php">View All Messages →</a>        </li>
        <li><a href="<?php echo $base_url; ?>">🌐 View Website</a></li>
        <li><a href="<?php echo $base_url; ?>auth/logout.php">Logout</a></li>
    </ul>
</aside>
